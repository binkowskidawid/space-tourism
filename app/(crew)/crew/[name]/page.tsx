...
const preload = (name: string) => {
	void getNameOfCrewMember(name);
};

export default async function CrewName({ params: { name } }: PageProps) {
	preload(name);
	const member = await getNameOfCrewMember(name);
	if (!member) {
		notFound();
	}
	const prev = previous(name, crew);
	const forward = next(name, crew);
	...
	return (
		<main className="container mx-auto min-h-[75vh] mt-16 flex lg:flex-row md:flex-col flex-col-reverse justify-between items-center">
			<div className="lg:w-1/2 lg:min-h-[75vh] flex-1 flex items-center justify-between">
				<button className="hidden md:block">
					<Link href={`/crew/${prev}`} className="transition duration-300">
				...
				<button className="hidden md:block">
					<Link href={`/crew/${forward}`} className="transition duration-300">
			...
			<div className="md:min-h-[60vh] lg:min-h-[90vh] md:w-full flex-1 flex flex-col items-center justify-between md:justify-end lg:pr-2">
				...
				<div className="w-[80vw] md:w-full border-b-2 border-slate-600 flex items-center justify-between md:justify-center">
